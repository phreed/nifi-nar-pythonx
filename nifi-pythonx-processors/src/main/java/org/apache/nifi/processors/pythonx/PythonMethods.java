 ...
 */
package org.apache.nifi.processors.pythonx;

/**
 * Class to initialize additional Python methods to work with SessionFile.java, Relationship, and Sessions easier
 */
class PythonMethods {
    ...
    static void init() {
        if (!initialized) {
            synchronized (PythonMethods.class) {
                if (!initialized) {
    ...
    private static boolean metaRelationship() {
        PythonSystem.getMetaClassRegistry().setMetaClass(Relationship.class, new DelegatingMetaClass(Relationship.class) {
            @Override
